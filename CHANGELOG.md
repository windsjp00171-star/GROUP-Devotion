...
### 新增
- `/history`：回顧月曆，往回翻小組排過的日子，點進去（`/history/<date>`）可以看那天的經文＋當天的動態牆，唯讀，不能在回顧頁面補寫領受。概念上參考天父日記「萬年曆」（一天一格、點進去看內容）,但不是照搬程式碼，也刻意重新設計成沒有打卡感的版本：格子有沒有點只代表「小組那天有沒有排經文」，不會標記「你今天有沒有寫」，每個人看到的每一格意義都一樣，不會變成完成率視覺化，符合北極星文件「不做進度條/完成率」的鐵律。
