...
- **安心使用聲明（`/about`）**：一頁不用登入就能看的聲明，講清楚這個系統做什麼、刻意不做什麼（沒有打卡／排行榜／催讀、不賣資料）、以及做了哪些資安管控（多小組隔離、session 不可偽造、cookie 加固、CSRF、CSV 防注入、RLS 預設拒絕、測試不碰正式站）。可以直接把這個網址貼出去給還沒登入的人先讀。
